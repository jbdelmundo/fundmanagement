...
class RequestEndorsementController extends Controller
{
    ...
    function index(Request $request){
        ...
        $rejects = [];
        foreach ($all_requests_this_sem as $key => $value) {
            $rejects[$key] = $value->where('status',Requests::REJECTED_ENDORSE);   //filter only those who is not yet processed
        }

    	return view('request_endorsement.index',compact('user','departments','department','aysem', 'requests_this_sem','endorsements','rejects'
            ));
    }
    ...
    function create(Request $formrequest){
        ...
        switch ($category) {
            ...
            case Requests::EBOOK:
                ...
                $validation_rules = ['subject'=>'required','quantity'=>'required|min:1|max:1' ];
                $this->validate($formrequest,$validation_rules);
                $request_endorsement = \App\RequestEndorsement::create($formrequest->toArray());                           

                break;

            case Requests::MAGAZINE:
            case Requests::JOURNAL:
            case Requests::ERESOURCE:

                $validation_rules = ['quantity'=>'required|min:1|max:1'];
                $this->validate($formrequest,$validation_rules);
                $request_endorsement = \App\RequestEndorsement::create($formrequest->toArray());

                break;

            //Equipment, supplies, others
            default:
  ...
  //       switch($category){
    ...
    function reject(Request $formrequest){        
        ...
        $category = $request->category_id;
        $request->status = Requests::REJECTED_ENDORSE;
